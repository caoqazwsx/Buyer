...
public class MinePresenter extends BasePresenter {
    ...
    public void getMyCommentList(final HttpCallbackListener listener){
        String address = Globalvariable.SERVER_ADDRESS+"type=select&sql="+Utility.encode("from Comment where commentAccount='"+Globalvariable.ACCOUNT+"'");
        getList(address,listener);
    }

    public void getMyCollect(final HttpCallbackListener listener){
        String address = Globalvariable.SERVER_ADDRESS+"type=mycollect&account="+Utility.encode(Globalvariable.ACCOUNT);
        getList(address,listener);
    }
    ...
    public  void addAddress( String contact, String telephone, String address,String no ,double lng,double lat,final HttpCallbackListener listener) {
      ...
      //前台数据获取
        ...
        try {
            tabledata.put("buyerAccount", Utility.encode(Globalvariable.ACCOUNT));
        ...
        String ads = Globalvariable.SERVER_ADDRESS+"type=insert&inserData=" + Utility.encode(indata.toString());
        update(ads,listener);
    }

    public void deleteAddress(int addressId,final HttpCallbackListener listener){
        String address = Globalvariable.SERVER_ADDRESS + "type=delete&sql="+Utility.encode("delete DeliveryAddress where id="+addressId);
        update(address,listener);
    }
    ...
    public  void editAddress( int addressId,String contact, String telephone, String address,String no ,double lng,double lat,final HttpCallbackListener listener) {
        String ads = Globalvariable.SERVER_ADDRESS + "type=update&sql="+Utility.encode(
